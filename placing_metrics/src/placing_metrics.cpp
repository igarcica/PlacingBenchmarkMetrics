...
////////// FIND CONTOURS AND APPROXIMATE POLYGON ///////////////
            ...
            {
                approxPolyDP(Mat(contours0[i]), siluette[i], 3, true);
                approxPolyDP(Mat(contours0[i]), contours[i], arcLength(contours0[i], true)*0.02, true);
...
///////////////////// FIND CORNERS //////////////////////////
            ...
            Point ptxmin, ptymin, ptxmax, ptymax, ptupleft, ptdownright;

            Point pt = contours[0][0];

            ptxmin = ptymin = ptxmax = ptymax = ptupleft = ptdownright = pt;
            xmin = xmax = pt.x;
            ymin = ymax = pt.y;
...
///////////////////// ROTATE CONTOUR //////////////////////////
            ...
            // Find angle of line
            double angle = atan2(ptupleft.y - ptdownright.y, ptupleft.x - ptdownright.x); //*180/PI
            
            //Rotate all the points of the SILUETTE
            vector<vector<Point>> rot_slt = siluette;
            for (size_t i = 0; i < siluette[0].size(); i++)
            {
                rot_slt[0][i].x = abs(siluette[0][i].x*cos(-angle) - siluette[0][i].y*sin(-angle));
                rot_slt[0][i].y = abs(siluette[0][i].x*sin(-angle) + siluette[0][i].y*cos(-angle));
            }

            //Draw the new polygon with the rotated points
            polylines(slt_img, rot_slt[0], true, Scalar(0,255,0));
//            imshow("Rotated siluette: ", slt_img);
...
int main(int argc, char** argv)
{
    ...
    // Load other images
    for( int i = 1; i<filenames.size(); i=i+1)
    {
